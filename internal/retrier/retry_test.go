...
func TestDo_NoRetryOnSuccess(t *testing.T) {
	SetDelays([]time.Duration{time.Millisecond})
	t.Cleanup(ResetDelays)
	called := 0
...
func TestDo_RetryUntilSuccess(t *testing.T) {
	SetDelays([]time.Duration{time.Millisecond, time.Millisecond})
	t.Cleanup(ResetDelays)
	attempts := 0
	err := Do(context.Background(), func() error {
		if attempts < 2 {
...
func TestDo_ContextCanceled(t *testing.T) {
	SetDelays([]time.Duration{time.Second})
	t.Cleanup(ResetDelays)
